...
# Filter the search results to only include the recommended handymen
filtered_results_all = [hm for hm in search_results if hm['id'] in recommended_handyman_ids]

# only take records with similarity scores greater than 0 and only take the first 10
filtered_results = [result for result in filtered_results_all if result['similarity'] > 0][:10]

# Write the filtered search results to standard output
